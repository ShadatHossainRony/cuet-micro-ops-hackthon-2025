Robust Asynchronous Download Service — SSE + Queue + S3
...
The current /v1/download/start endpoint performs a synchronous, long-running operation (10–120s) and returns the final file in the same HTTP request. Reverse proxies and load balancers commonly terminate such requests, resulting in client-facing errors, resource exhaustion, and poor UX. The solution is to rework the request into an asynchronous job flow with real-time progress delivered via SSE and final delivery via presigned S3 URL.
...
## API Contract (Revised)

### POST /v1/download/start

_Description:_ Initiate processing for a requested file. This endpoint replaces the previous synchronous behavior with immediate job creation.
_Request Body:_
...
_Description:_ Open a Server-Sent Events stream for job progress. The server will emit structured events until job completion or failure.
_Event Types:_ queued, started, progress, completed, failed
_Event payload example:_
...
_Description:_ Fetch metadata and a presigned S3 URL for download when available. Returns 404 if job is unknown, 202 if job still processing, and 200 with url when ready.
_Responses:_
...
_Queue:_ Redis + BullMQ
_Workers:_ One or more worker processes that:
...
_SSE Emitter:_ The API service exposes an SSE endpoint and maintains event channels per jobId.
...
_Tracing:_ Instrument API and workers with OpenTelemetry. Capture spans for:
...
_Error Tracking:_ Integrate Sentry in both API and worker processes. Capture exceptions, record jobId and trace IDs in error metadata.

_Metrics:_ Expose Prometheus metrics or similar for:
...
1. _Client disconnects mid-processing_ — job continues; client can reconnect and resume updates via SSE and GET /v1/download/:jobId.
2. _Worker crashes_ — retries via queue; failed jobs move to dead-letter queue and emit failed event where possible
3. _S3 upload fails_ — retry upload with backoff; on persistent failure mark job as failed and alert via Sentry
4. _Proxy kills connections_ — SSE reduces this risk; presigned URLs remove streaming of large files
5. _Duplicate work due to retries_ — make workers idempotent and use atomic operations for final object publish

---

## Operational Notes and Deployment
...
Client->>API: POST /v1/download/start {file_id}
API-->>Client: 200 {jobId, status: queued}
API->>Redis: enqueue job(jobId, file_id)
Client->>API: GET /v1/download/events/:jobId (SSE)
Redis-->>Worker: deliver job
Worker->>Redis: update status processing
Worker->>API: publish progress -> Redis pubsub
API->>Client: SSE event progress
Worker->>S3: upload final artifact
S3-->>Worker: confirm upload
Worker->>Redis: update status completed, set s3Key
API->>Client: SSE event completed
Client->>API: GET /v1/download/:jobId
API-->>Client: 200 {url: presigned_s3_url}
Client->>S3: GET presigned_url (direct download)
...
API[Hono API]
SSE[Events / SSE Handler]
end

subgraph Backend
Redis[(Redis + BullMQ)]
Worker[Worker Pool]
ObjectStorage[(S3 - MinIO / RustFS)]
end
...
UI -->|POST /start| API
UI -->|SSE /events/:jobId| SSE
API -->|enqueue| Redis
Worker -->|pop jobs| Redis
Worker -->|upload| ObjectStorage
API -->|presigned URL request| ObjectStorage
API --> Jaeger
Worker --> Jaeger
API --> Sentry
Worker --> Sentry
